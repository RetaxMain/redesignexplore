    ...
    <div className={`bg-white rounded-lg shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden border group hover:border-gray-200 h-full flex flex-col ${selected ? 'border-blue-500 ring-2 ring-blue-100' : 'border-gray-100'}`}>
      <div className="relative h-32 overflow-hidden bg-gradient-to-br from-gray-100 to-gray-200">
        ...
        <div className="absolute top-2 right-2 bg-white/95 backdrop-blur-sm rounded-lg px-2 py-1 flex items-center shadow-sm">
          ...
          <span className="text-xs font-semibold text-gray-800">{school.rating ?? 'N/A'}</span>
        </div>
      ...
      <div className="p-3 flex-1 flex flex-col">
        <div className="mb-3">
          ...
          <div className="flex items-center text-gray-500 text-xs">
            ...
            <span className="truncate">
              {school.location?.city || 'N/A'}, {school.location?.state || 'N/A'}
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2 mb-3 p-2 bg-gray-50 rounded-lg">
          <div className="text-center">
            <div className="flex items-center justify-center mb-1">
              ...
              <span className="text-xs font-semibold text-gray-900">{school.students ? (school.students / 1000).toFixed(1) + 'K' : 'N/A'}</span>
            </div>
          ...
          <div className="text-center">
            <div className="flex items-center justify-center mb-1">
              ...
              <span className="text-xs font-semibold text-gray-900">{school.establishedYear ?? 'N/A'}</span>
            </div>
          ...
          <div className="text-center">
            <div className="flex items-center justify-center mb-1">
              ...
              <span className="text-xs font-semibold text-gray-900">{school.achievements?.length ?? 'N/A'}</span>
            </div>
            <span className="text-xs text-gray-600">Awards</span>
          </div>
        </div>

        {typeof school.tuitionFee === 'number' ? (
          ...
          <div className="flex items-center justify-between mb-3 p-2 bg-blue-50 rounded-lg">
            ...
            <span className="text-xs font-bold text-blue-900">N/A</span>
          </div>
        )}
        ...
        <p className="text-gray-600 text-xs mb-3 line-clamp-2">{school.description || 'N/A'}</p>
