...
import Head from 'next/head'; // Still works, but consider Metadata API for App Router SEO
import {
    ArrowLeft,
    MapPin,
    Users,
...
import { School } from '../../types/school'; // Adjust path if necessary
import { mockSchools } from '../../data/mockSchools'; // Adjust path if necessary
import ContactModal from '../../components/ContactModal'; // Adjust path if necessary
import { useRouter, useSearchParams, useParams } from 'next/navigation'; // Import useParams and useSearchParams

// Removed the props interface here, as these props are not directly passed to a client component.
// Instead, we'll use hooks.
// interface SchoolPageProps {
//     params: { slug: string };
//     searchParams: { id?: string };
// }

export default function SchoolPage(/* No props needed here for a client component getting params via hooks */) {
    const router = useRouter();
    const searchParams = useSearchParams(); // Use the hook for query parameters
    const params = useParams(); // Use the hook for dynamic route parameters (e.g., slug)
    ...
    // Access properties from the hook returns
    const schoolId = searchParams.get('id') || (params.slug as string); // use .get() for searchParams
    // params.slug will be string | string[], cast if confident it's string
    ...
    return (
        <>
            <Head>
                ...
                <link rel="canonical" href={`https://schoogle.com/school/${params.slug || schoolId}`} />
            </Head>
